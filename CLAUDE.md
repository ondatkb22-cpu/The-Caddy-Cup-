...
This is a **single-file static web app**. The entire product lives in `index.html` (~3K lines). There is no build system, no `package.json`, no dependencies, no tests, and no server. The README is intentionally minimal (`caddy cup / Caddy Champ dfs optimizer`).
...
| Lines | Region | Notes |
|------:|--------|-------|
| 8–1486 | `<style>` | Design tokens (`:root` CSS vars at top, line ~15), mobile-first layout, dark theme. Three media queries: `min-width: 768px` (desktop scale-up), `min-width: 1200px` (wide), `max-width: 767px` (mobile — card layout for the player pool, sticky-stack reduction, enlarged touch targets, scroll-fade on tabs/filters) |
...
- **Event handling**: a single delegated click handler on `document.body` (line ~2513) dispatches on `data-lock`, `data-exclude`, `data-unlock`, `data-unexclude`, `data-remove`, `data-load-lineup`, `.row-check`, and player rows. The `.row-check` and row-expand branches use `e.target.closest(...)` so clicks land correctly on nested children (e.g. the card's "+ Add" button). Add new interactive controls by adding a `data-*` attribute and a branch in this handler rather than per-element listeners.
...
`SIG_EVENTS` (line ~2002) is a hardcoded mapping of player → signature-event finishes, with `SIG_YEAR_WEIGHTS` weighting current season at 1.0 and prior at 0.6. This drives the "Signature Event Specialists" card on the Top Plays tab.

## CSS conventions

CSS variables in `:root` (line ~15) define spacing, surfaces, borders, text colors, brand/sentiment colors, tier colors, and touch-target sizing (`--touch-min`, `--card-gap`). Use the tokens (`var(--bg-1)`, `var(--accent)`, `var(--tier-sharp)`, etc.) rather than literal colors. The design intentionally avoids decoration — dense data, breathable spacing, "Stokastic/4for4 aesthetic" per the comment at the top of the stylesheet.

**ASCII only inside `<style>` and `<script>`.** A previous text-formatter run silently replaced `--` with U+2013 EN DASH and `"`/`'` with curly quotes, breaking every CSS custom property in the file. Don't paste through editors that auto-correct these. If you suspect mangling, run `grep -c $'\xe2\x80\x93' index.html` — anything > 0 in lines 8–1486 is broken CSS.
...
- Avoid `grep`/`awk` over the whole file from Bash — line 1491's embedded JSON is enormous and floods stdout. Search with line-number-anchored Read calls or grep with `head` limits.
...
- For mobile changes: there are no automated tests. Verify by walking the viewport range 320 / 360 / 390 / 414 / 768 / 1024 in DevTools and exercising lock / fade / add / expand / optimize / clear flows.
